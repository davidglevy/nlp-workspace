...
from pyspark.sql.types import StructType, StructField, StringType, MapType, ArrayType, BinaryType, IntegerType
...
def nlp_urls():
    ...
    return spark.createDataFrame(url_list, schema_load)
...
@dlt.table
def audio_raw():
    downloaded_df = dlt.read("nlp_urls").repartition(20).withColumn("downloaded", download_audio_udf(col("url"), col("file_name")))
    final_df = downloaded_df.select(
        "title",
        "date",
        "downloaded.path", 
